...
<%
	if (request.getParameter("codigo") == null) {
		response.sendRedirect("TipoDocumento.jsp");
		return;
	}
	
	String codigo = (String)request.getParameter("codigo");
	db conexion = new db();
	ControladorTipoDocumento manejador = new ControladorTipoDocumento(conexion);
	if (!manejador.find(codigo)) {
		response.sendRedirect("TipoDocumento.jsp");
		return;
	}
	
	TipoDocumento tipo = manejador.getTipo();
...
<script type="text/javascript">
	...
	function procesarEliminar(resultado) {
		...
		if (resultado != 'OK') {
			...
			window.location.href = "TipoDocumento.jsp";
		}
...
<p>
	Esta vista permite modificar el registro de Tipo de documento.  Debe diligenciar la informacion y dar clic en el boton Actualizar.
</p>
<div id="info"></div>
