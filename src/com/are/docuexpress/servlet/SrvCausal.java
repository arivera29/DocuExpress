 ...
 * @author Aimer
 */
@WebServlet("/SrvCausal")
public class SrvCausal extends HttpServlet {
            ...
            try {
                ...
                */
                if (operacion.equals("add")) { 
                    ...
                    ControladorCausal controlador = new ControladorCausal(conexion);
                    
                    if (!controlador.find(codigo)) {
                        Causal causal = new Causal();
                        causal.setCodigo(codigo);
                        causal.setNombre(nombre);
                        causal.setEstado(Integer.parseInt(estado));
                        
                        if (controlador.add(causal)) {
                            out.print("OK");
                        }else {
                            out.print("Error al agregar el registro");
                        }
                    }else {
                        out.print("Codigo de Causal ya se encuentra registrado");
                    }
                }
                /*
                ...
                */
                if (operacion.equals("update")) { 
                    ...
                    if (controlador.find(key)) {
                        ...
                        causal.setNombre(nombre);
                        causal.setEstado(Integer.parseInt(estado));
                        if (controlador.update(causal,key)) {
                            out.print("OK");
                        }else {
                            out.print("Error al actualizar el registro");
                        }
                    }else {
                        out.print("Codigo de Causal no se encuentra registrado");
                    }
                }
                ...
                */
                if (operacion.equals("list")) { 
                    conexion = new db();
                    ControladorCausal controlador = new ControladorCausal(conexion);
                    ArrayList<Causal> lista = controlador.list();
                    ...
                    out.println("<tbody>");
                    for (Causal causal : lista) {
                        out.println("<tr>");
                        out.println("<td>" + causal.getCodigo() + "</td>");
                        out.println("<td>" + causal.getNombre() + "</td>");
                        out.println("<td>" + causal.getEstado() + "</td>");
                        out.println("<td><a class='btn btn-primary bt-sm' href='mod_causal.jsp?codigo=" + causal.getCodigo() + "'>Editar</a></td>");
                        out.println("</tr>");
                    }
                    out.println("</tbody>");
                    out.println("</table>");
                ...
                if (conexion != null) {
                    try {
                        ...
                        Logger.getLogger(SrvCausal.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
